...
| `set-read-capture <seconds> [--start-offset <seconds>]` | `SetReadCapture` | Configure the automatic read-back: how many seconds to read, and the offset **from the start of the capture** (not a wall-clock time) at which to begin. Overwrites `common/read_config.ini`. |
| `set-read-rate <hz>` | `SetReadCaptureRate` | Set the receiver sample rate (Hz) used to size the read-back. Overwrites `common/read_config.ini`. |
...
`common/read_config.ini` to the same file.** By default this is a path
...
starting them:
```bash
export XRCOMM_READ_CONFIG="/absolute/path/to/XRComm_FCS_wideband_platform/common/read_config.ini"
```
