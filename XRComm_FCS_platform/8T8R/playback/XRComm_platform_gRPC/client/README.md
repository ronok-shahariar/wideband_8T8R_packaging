...
## 2. Generate the stubs (not shipped — generated from the .proto)

From `XRComm_platform_gRPC/`:

```bash
./generate_stubs.sh
```

This writes `pipeline_control_pb2.py` and `pipeline_control_pb2_grpc.py` into
...
| GetChannelFilename / SetChannelFilename | ChannelRef / ChannelFilenameValue | …Value / PlaybackConfig | One channel's `ch<N>_filename` (N=0..7; "" disables) |
